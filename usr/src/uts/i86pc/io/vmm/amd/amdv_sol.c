...
dev_info_t *
ivrs_get_dip(ACPI_IVRS_HARDWARE *ivrsh, int unit)
{
	dev_info_t *dip;
	...
	dip = ddi_find_devinfo("ivrs", unit, 0);
	...
	/*
	 * None found, construct a devinfo node for this ivrs unit.
	 */
	dip = ddi_add_child(ddi_root_node(), "ivrs",
	    DEVI_SID_NODEID, unit);

	reg.regspec_bustype = 0;
	reg.regspec_addr = ivrsh->BaseAddress;
	...
	pdptr->par_reg->regspec_addr = ivrsh->BaseAddress;
